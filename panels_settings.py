...
(disconnect per CircleCI account) for CircleCI Connector. Split out of
panels.py per the same convention as MuleSoft Connector's / Power Automate
...
def _connection_row(c: dict) -> ui.UINode:
    ...
    return ui.Stack(direction="v", gap=1, align="start", children=[
        ...
        ui.Button(
            ...
            on_click=ui.Call("disconnect_circleci", {"connection_id": c.get("id")}),
        ),
    ])
...
def _connections_section(connections: list[dict]) -> ui.UINode:
    if not connections:
        return ui.Stack(direction="v", gap=1, children=[
            ...
            ui.Text("No CircleCI accounts connected yet.", variant="caption"),
        ])
...
@ext.panel("circleci_settings", slot="center")
async def circleci_settings_panel(ctx) -> ui.UINode:
